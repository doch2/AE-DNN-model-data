...
for imageKind in ["train", "test"]:
    removeFile(imageFolderPath + imageKind + "_dataFrame.csv")

    imageList = os.listdir(imageFolderPath + "image_" + imageKind + "/dayChart/dayChart1/")
    imageList.sort()
    ...
    for imageName in imageList:
        imageDate = imageName[imageName.find('_')+1:imageName.find('.jpg')]
        ...
        progress = (round((imageList.index(imageName) + 1) / len(imageList), 4) * 100)
        clearConsole()
        print("\nKOSPI 시장의 " + imageKind + "을(를) 진행할 차트 사진 리스트 로딩이 완료되었습니다. | 일별(시간봉) 차트 사진 개수: " + str(len(imageList)))
